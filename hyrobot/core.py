...
class  SuiteFileConvert:
        ...
        for level2 in classNode.body:
            ...
            if type(level2) == ast.Assign:
                ...
                if type(target) == ast.Name:
                    ...
                    if  target.id == 'cases_backend' and type(value) == ast.List:
                        testcase['cases_backend'] = ast.literal_eval(value)
    ...
    def writeRobotSuiteFile(self):
        ...
        for tc in self.suite['testcases']:
            if not ('name' in tc   or   'cases_backend' in tc):
                print(f'!! {tc["classname"]}没有 name 或者 cases_backend 定义')
                continue
        ...
        for tc in effective_testcases:
            ...
            #         case, username, password = self.cases_backend[index]

            elif tc['type'] == 'multicase':
                for idx, case in enumerate(tc["cases_backend"]):
                    casename = case[0]
                    testcases_txt += f'\n\n{casename}\n'
